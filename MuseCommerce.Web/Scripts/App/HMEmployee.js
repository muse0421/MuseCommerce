...
var ngWebUploader = angular.module('ngWebUploader', []);
ngWebUploader.factory('myWebUploader', [function () {
    var myWebUploader2 = WebUploader;
    return myWebUploader2; // assumes underscore has already been loaded on the page
}]);

var AppDependencies = ['ui.router', 'ngAnimate','ngdatepicker','ngWebUploader'];
...
app.controller('orgcodeCtrl', function ($scope, $http) {
    ...
    $stateProvider
        .state('index', {
            url: '/index',
            views: {
                '': { templateUrl: '/Scripts/App/tpls/HMEmployee/index.tpl.html' }
            }
        })
        .state('create', {
            url: '/create',
            views: {
                '': { templateUrl: '/Scripts/App/tpls/HMEmployee/create.tpl.html' }
            }
        }).state('edit', {
            url: '/edit/{ID}',
            views: {
                '': { templateUrl: '/Scripts/App/tpls/HMEmployee/edit.tpl.html' }
            }
        })
        .state('display', {
            url: '/{ID}',
            views: {
                '': { templateUrl: '/Scripts/App/tpls/HMEmployee/display.tpl.html' }
            }
        });
...
app.controller('IndexorgcodeCtrl', function ($scope, $http, $state, $stateParams) {
    ...
    $scope.search = function () {
        ...
        $http.get("/HR/HMEmployee/HMEmployeeInfo", config)
    ...
    $scope.delete = function (fid) {
        ...
        swal({
            ...
            $http.delete("/HR/HMEmployee/DeleteHMEmployee", config)
            .success(function (response) {
    ...
    $scope.forbidden = function (fid) { 
        var data = { 'fid': fid };
        $http.put("/HR/HMEmployee/forbidden",data)
           .success(function (response) {
               $scope.success = response.success;
               $scope.search();
        });
    ...
    $scope.restore = function (fid) {       
        var data = { 'fid': fid };
        $http.put("/HR/HMEmployee/restore", data)
           .success(function (response) {
               $scope.success = response.success;
               $scope.search();
           });
...
app.controller('EditorgcodeCtrl', function ($scope, $http, $state, $stateParams, $rootScope, mydatepicker, myWebUploader) {
    ...
    var uploader = WebUploader.create({
        ...
        // 文件接收服务端。
        server: '/HR/HMEmployee/UploaderPhoto?Employeeid=' + $stateParams.ID,
        // 选择文件的按钮。可选。
    ...
    uploader.on('uploadSuccess', function (file) {
        ...
        $('#employeephoto').attr('src', '/img/' + $stateParams.ID + '.jpg?time=Math.random()');
        console.log('已上传');
    });
    ...
    $scope.loadorgcode = function () {
        var config = { params: { 'qname': '', 'start': '0', 'length': '1000' } };

        $http.get("/MGCode/ORGDepartment/ORGDepartmentInfo", config)
        .success(function (response) {
            $scope.ORGDepartmentInfo = response.data;
            $http.get("/MGCode/ORGPDescriptionDuty/ORGPDescriptionDutyInfo", config)
           .success(function (response) {
               $scope.ORGPDescriptionDutyInfo = response.data;
               config = { params: { 'ID': $stateParams.ID } };
               $http.get("/HR/HMEmployee/HMEmployee", config)
    ...
    $scope.save = function () {
        ...
        //console.log(data);
        $http.put("/HR/HMEmployee/PutHMEmployee", data, config)
        .success(function (response) {
            $scope.success = response.success;
...
app.controller('CreateorgcodeCtrl', function ($scope, $http, $state, $stateParams, $rootScope, mydatepicker) {
    mydatepicker('[data-provide="datepicker-inline"]').datepicker();
        ...
        .success(function (response) {
           ...
           .success(function (response) {
               ...
               $scope.orgcode = { 'FNumber': "", "FName": "", "FDepartmentID": "", "FDutyID": "", "FHireDate": "" };
    ...
    $scope.save = function () {
        ...
        var config = {};
        var data = $scope.orgcode;
        $http.post("/HR/HMEmployee/SaveHMEmployee", data, config)
        .success(function (response) {
            $scope.success = response.success;           
            $rootScope.$state.go('index');
        });
